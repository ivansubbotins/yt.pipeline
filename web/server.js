...
- Релевантные теме и содержанию видео`;
  ...
  if (pathname === '/api/express/cover-prompts' && req.method === 'POST') {
    try {
      ...
      // User-supplied trigger texts (one per cover). If provided, Claude MUST use them verbatim
      // instead of inventing its own text_on_image.
      const userTriggers = Array.isArray(body.triggers)
        ? body.triggers.map(s => String(s || '').trim()).filter(Boolean).slice(0, count)
        : [];

      console.log('[Cover Prompts] Generating prompts, topic:', topic, 'count:', count,
                  userTriggers.length ? 'user-triggers: ' + userTriggers.length : 'auto-triggers');

      const triggerInstruction = userTriggers.length
        ? `\nПОЛЬЗОВАТЕЛЬСКИЕ ТЕКСТЫ ДЛЯ ОБЛОЖЕК (используй их БУКВАЛЬНО, по одному на обложку, в том же порядке):
${userTriggers.map((tr, i) => `  ${i+1}. "${tr}"`).join('\n')}
В поле text_on_image для обложки N — поставь ТОЧНО текст N из списка выше, без изменений.`
        : `\nГенерируй text_on_image сам. Жёсткие требования:
- ТОЛЬКО кириллица (русский язык). Английские слова ЗАПРЕЩЕНЫ.
- 2-4 слова, КАПСОМ, эмоциональный триггер.
- Используй формулы: шок-цифра ("28 МЛН ЗА ГОД"), провокация ("ВРАЧ СОЛГАЛ"), интрига ("СЕКРЕТ #1"),
  противопоставление ("ДО/ПОСЛЕ"), вопрос ("А ТЫ ТАК МОЖЕШЬ?"), боль ("ХВАТИТ ТЕРЯТЬ"), результат ("+5 МЛН"),
  негатив ("ЭТО ЛОЖЬ"), список ("3 ОШИБКИ"), уникальность ("НИКТО НЕ ЗНАЕТ").`;
...
ABSOLUTE RULE: text_on_image MUST be in CYRILLIC (Russian). NEVER English.
ABSOLUTE RULE: any banknotes/money in the scene MUST be Russian rubles — 5000-ruble notes (фиолетовые/сиреневые купюры с памятником Хабарову, не доллары, не евро). Mention this explicitly in the visual prompt.`,
        `Video title: "${title}"
...
[
  {
    "prompt": "Full English prompt describing the visual scene (person pose, background, lighting, mood, effects). If money/banknotes appear in the scene, write 'Russian 5000-ruble banknotes (purple Russian rubles, NOT US dollars)'.",
    "text_on_image": "2-4 СЛОВА КАПСОМ НА КИРИЛЛИЦЕ (русский язык). Никакого английского.",
...
REMINDER:
- text_on_image — ОБЯЗАТЕЛЬНО кириллица, никакого латинского алфавита
- Если в сцене есть деньги — это всегда РУБЛЁВЫЕ купюры (5000₽, фиолетово-сиреневые), НЕ доллары и НЕ евро`
      );
      ...
      // Post-process: if user supplied triggers, force them onto the prompts even if
      // Claude ignored the instruction. Trust the user's text over Claude.
      if (userTriggers.length) {
        prompts = prompts.map((p, i) => userTriggers[i]
          ? { ...p, text_on_image: userTriggers[i] }
          : p);
      } else {
        // If Claude returned latin chars in text_on_image despite our rule, flag it
        prompts = prompts.map(p => {
          const txt = p.text_on_image || '';
          // If text is mostly Latin, fall back to short uppercase topic snippet
          const cyrillicChars = (txt.match(/[А-ЯЁа-яё]/g) || []).length;
          const latinChars = (txt.match(/[A-Za-z]/g) || []).length;
          if (latinChars > cyrillicChars && latinChars > 2) {
            console.warn('[Cover Prompts] Claude returned latin text_on_image:', txt, '— keeping but flagging');
            return { ...p, text_on_image: txt, _latin_warning: true };
          }
          return p;
        });
      }
